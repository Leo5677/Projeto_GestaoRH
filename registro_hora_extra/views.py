...
class HoraExtraBaseUpdateView(CreateView):
    model = RegistroHoraExtra
    form_class = RegistroHoraExtraForm
    success_url = reverse_lazy('list_hora_extra')

    def get_form_kwargs(self):
        kwargs = super(HoraExtraBaseUpdateView, self).get_form_kwargs()
...
class NaoUtilizouHoraExtraView(View):
    def post(self, *args, **kwargs):
        ...
        response = json.dumps(
            {'mensagem': 'Hora Extra Recuperada',
             'horas_nao_utilizadas': float(funcionario.total_hora_extra)}
        )
...
class ExportarCsv(View):
    def get(self, request):
        ...
        for registro in registro_he:
            writer.writerow(
                [registro.id, registro.motivo, registro.funcionarios,
                 registro.funcionarios.total_hora_extra, registro.horas
                 ])
...
class ExportarExcel(View):
    def get(self, request):
        ...
        for registro in registros:
            ...
            ws.write(row_num, 2, registro.funcionarios.nome, font_style)
            ws.write(row_num, 3, registro.funcionarios.total_hora_extra, font_style)
